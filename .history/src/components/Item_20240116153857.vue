  ...
  <ion-grid>
    <ion-row v-for="(item, index) in items" :key="index">
      ...
      <ion-row>
        ...
        <!-- 小計の表示 -->
      <ion-row v-if="(items.indexOf(item) +1) % ITEM_COLOR_COUNT.value === 0" class="subtotal-row">
...
<script>
import { ITEM_COLOR_COUNT } from '@/modules/MyModule.vue';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const calculateSubtotal = (item) => {
      ...
      const start = index - (index % ITEM_COLOR_COUNT.value);
      const end = index;
