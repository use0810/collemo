...
import { defineComponent, inject, onMounted, ref, watch } from 'vue';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const targetObject = pageInfo.value.list.find(item => item.name.includes(toggle.name));
      if (targetObject) {
    targetObject.valid = toggle.valid;
    console.log(`Object with name '${toggle.name}' found. Updated valid to ${toggle.valid}`);
  } else {
    console.log(`Object with name '${toggle.name}' not found.`);
  }
    };
