...
export default {
  async drawTable(collemo, name){
    ...
    // const item_counts = collemo.list.map((item,index) => item.count);

    const item_validats = collemo.list.map(item => { 
      return item.valid === false? false : true;
    })
    const item_counts = collemo.list.map((item, index) => {
      return item_validats[index] ? item.count : 0;
    });
    let item_sum =[];
    let temp = 0;
    ...
    cells_x.forEach((x_point, x_index) => {
      ...
      cells_y.forEach((y_point, y_index) => {
        ...
        if(item_validats[x_index*11] == false && batsu_flg==false) {
          ctx.moveTo(cells_x[x_index], cells_y[12]);
          ctx.lineTo(cells_x[x_index]+cell_w, cells_y[11]+cell_h);
          ctx.stroke();
          console.log(batsu_flg)
          batsu_flg = true;
